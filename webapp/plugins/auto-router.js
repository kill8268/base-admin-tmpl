...
export default function AutoRouterPlugin({
  ...
  const toRouter = (filepath, isDirectory, islayoutFile) => {
    ...
    const url =
      path.replace(ext, "").replace("/index", "").replace("index", "") || "/";
    if (isDirectory) {
      const route = {
        path: toRouterPath(url),
        element: islayoutFile
          ? Path.join(path, layoutFile).split(Path.sep).join("/")
          : undefined,
      };
      return route;
    }
  ...
  const getTree = (url = "") => {
    ...
    if (stat.isDirectory()) {
      ...
      route.children = files
        ...
        .filter(
          (item) => !item.includes(layoutFile) && !separateFileSet.has(item),
        ) // 过滤掉指定的独立层级文件
